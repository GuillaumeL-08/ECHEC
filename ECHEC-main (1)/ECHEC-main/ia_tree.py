...
class TreeIA:
    def __init__(self, depth=2, enable_learning=True):
        self.depth = depth
    ...
    def evaluate(self) -> int:
        """Évaluation avancée de la position avec apprentissage."""
        ...
        # 7) Intégrer l'apprentissage si activé
        if self.learning_manager:
            score = self.learning_manager.get_position_value_with_learning(self.board, score)
    ...
    def coup(self, board) -> str:
        ...
        # Exploration vs Exploitation pour l'apprentissage
        if self.learning_manager and self.learning_manager.should_explore():
            # Exploration: choisir un coup aléatoire parmi les meilleurs
            moves = list(self.board.legal_moves)
            if moves:
                move = random.choice(moves)
                score = self.evaluate()
                if self.learning_manager:
                    self.learning_manager.record_move(self.board, move, score)
                return self.board.san(move)
        
        # Exploitation: utiliser le meilleur coup trouvé
        _, move = self.minimax(self.depth, -10**9, 10**9, maximizing)
        if move is None:
            raise ValueError("Aucun coup trouvé")
        
        # Enregistrer le coup pour l'apprentissage
        if self.learning_manager:
            score = self.evaluate()
            self.learning_manager.record_move(self.board, move, score)
    ...
    def end_game(self, result: str, board=None):
        ...
        if self.learning_manager:
            # Utiliser le board fourni ou l'attribut board s'il existe
            final_board = board if board is not None else getattr(self, 'board', None)
            if final_board is None:
                final_board = Board()  # Board par défaut si aucun disponible
            self.learning_manager.end_game(result, final_board)
            # Préparer pour la prochaine partie
            self.learning_manager.start_new_game()
        
        # Réinitialiser le compteur d'ouvertures
